    ...
    return (
        <>
            ...
            <div className="my-4 text-right">
                ...
                <div>delivery {decimalToMoney(shippingCost)}</div>
                <div className="font-bold">Total: {decimalToMoney(subtotal + shippingCost)}</div>
            </div>

            {auth.token &&
                <Button onClick={handleFinish} className="bg-green-700 hover:bg-green-900">Finalizar Pedido</Button>
            }
            {!auth.token &&
                <Button onClick={() => auth.setOpen(true)} className="w-full">Login / Cadastro</Button>
            }
        </>
    )
}